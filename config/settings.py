...
class Settings(BaseSettings):
    ...
    openrouter_timeout: int = Field(default=60, description="OpenRouter 타임아웃(초)")
    openrouter_max_retries: int = Field(
        default=3, description="OpenRouter 최대 재시도 횟수"
    )
    default_model: str = Field(
        default="anthropic/claude-sonnet-4", description="기본 LLM 모델"
    )

    # Ollama 설정 (레거시 호환용)
    ollama_api_url: str = Field(
        default="http://localhost:11434", description="Ollama API URL (레거시)"
    )
    ...
    rate_limit_window: int = Field(
        default=60, description="레이트 리미트 시간 윈도우(초)"
    )
